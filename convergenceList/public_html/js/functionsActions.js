                                        ...
                                        success: function(o, resp) {
                                            ...
                                            if (typeof(dataCSV.success) != 'undefined' && dataCSV.success === true) {
                                                ...
                                                winMatchData.on('hide', function() {
                                                    if (Ext.getCmp(_dblIdMainGrid))
                                                        Ext.getCmp(_dblIdMainGrid).getStore().reload();
                                                });
                                            } else {
...
function listeTransactions(porteurId)
{
    var adaptiveHeight = getDocHeight() - 50;
    ...
    var _CLOSE = 'Fermer';
    var _NO_PROD = 'Aucune transactions';
    var _WINTITLE_DOUBLON = "Liste des transactions";
    ...
    column = {
        id: 'UID',
        header: 'N&deg; transaction',
        width: 90,
        dataIndex: 'UID'
    };
    _dblColumns.push(column);
    _dblFields.push({name: 'UID'});
    ...
    _dblColumns.push(column);
    _dblFields.push({name: 'DATE_EMISSION'});
    ...
    _dblColumns.push(column);
    _dblFields.push({name: 'DATE_VALIDATION'});

    column = {
        id: 'DATE_ENVOI',
        header: 'Date d\'envoi au remboursement',
        width: 120, //30,req
        dataIndex: 'DATE_ENVOI'

    };
    _dblColumns.push(column);
    _dblFields.push({name: 'DATE_ENVOI'});

    column = {
        id: 'DATE_REMBOURSEMENT',
        header: 'Date de remboursement',
        width: 120, //30,req
        dataIndex: 'DATE_REMBOURSEMENT'

    };
    _dblColumns.push(column);
    _dblFields.push({name: 'DATE_REMBOURSEMENT'});
    ...
    _dblColumns.push(column);
    _dblFields.push({name: 'MONTANT'});

    storeTransactions = new Ext.data.JsonStore({
        url: '../convergenceList/actions/listeTransactions.php?porteurId=' + porteurId,
        root: 'data',
        totalProperty: 'total',
        autoWidth: true,
        fields: _dblFields
    });
    storeTransactions.load();
